...
const ingredientItemSchema = z.object({
  name: z
    .string()
    .describe(
      'English-translated ingredient name, capitalized (e.g. "Sugar", "Palm Oil")',
    ),
  status: z
...
function buildProfileContext(onboarding: OnboardingResponse): string {
  ...
  if (onboarding.nutritionPriorities.length > 0) {
    parts.push(
      `Nutrition priorities: ${onboarding.nutritionPriorities.join(', ')}`,
    );
  }
...
${profileContext}`;
  ...
  console.log(
    `[IngredientAnalysis] Starting — ${product.ingredients?.length ?? 0} ingredients`,
  );
  ...
  try {
    ...
    const structured = model.withStructuredOutput(
      ingredientAnalysisOutputSchema as any,
    );
    ...
    console.log(
      `[IngredientAnalysis] Done — ${result.ingredients.length} items  ${Date.now() - start}ms`,
    );
    return result;
  } catch (err) {
