...
const runAnalysisJob = async (
  ...
  try {
    ...
    console.log(
      `[Job:${jobId}] 👤 Profiles (${profiles.length}): ${profileNames}`,
    );

    // Step 2: Get nutrition facts (product data first, web search fallback)
    const productHasNutrition = hasNutritionData(product);
    console.log(
      `[Job:${jobId}] 📊 Product has nutrition data: ${productHasNutrition}`,
    );

    // Step 3: AI classification + nutrition resolution + ingredient analysis (all parallel)
    console.log(
      `[Job:${jobId}] 🧪 Step 1 — Extracting classification${productHasNutrition ? '' : ' + searching nutrition'} + ingredients...`,
    );
    ...
    const ingredientPromises = profiles.map((profile) =>
      analyzeIngredients(product, profile.onboarding).catch((err) => {
        console.warn(
          `[Job:${jobId}] ⚠️ Ingredient analysis failed for "${profile.name}":`,
          err,
        );
        return null;
      }),
    ...
    if (productHasNutrition) {
      ...
      const [classificationResult, ...ingResults] = await Promise.all([
        getProductFactsService()
          ...
          .catch(() => {
            console.warn(
              `[Job:${jobId}] ⚠️ AI classification failed, using deterministic fallback`,
            );
      ...
      if (webNutrition) {
        ...
        console.log(
          `[Job:${jobId}] ⚠️ No nutrition found anywhere — scoring with empty data`,
        );
      }
    }
    ...
    for (let i = 0; i < profiles.length; i++) {
      perProfileIngredients.set(
        profiles[i].profileId,
        ingredientResults[i] ?? null,
      );
    }

    // Step 4: Merge classification + nutrition → ProductFacts
    const facts: ProductFacts = buildProductFacts(
      classification,
      nutritionFacts,
    );
    console.log(
      `[Job:${jobId}] ✅ Facts built  ${Date.now() - factsStart}ms  type=${facts.productType}`,
    );
    ...
    const profileScores = computeAllProfileScores(
      facts,
      profiles,
      perProfileIngredients,
    );
    console.log(
      `[Job:${jobId}] ✅ Scores computed  ${Date.now() - scoreStart}ms`,
    );

    for (const ps of profileScores) {
      console.log(
        `[Job:${jobId}]   "${ps.name}" → score=${ps.score} (${ps.fitLabel})  +${ps.positives.length}✓ -${ps.negatives.length}✗`,
      );
    }
    ...
    console.log(
      `[Job:${jobId}] 🏁 Job completed  total=${Date.now() - jobStart}ms`,
    );
  } catch (err) {
