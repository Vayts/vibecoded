...
function getUnsaturatedFatRatioScore(product: NormalizedProductV2): number | null {
  const fat = product.nutrition.fatPer100g;
  const saturatedFat = product.nutrition.saturatedFatPer100g;
...
export function buildNutrientSubScores(
  ...
  const caloriesPerServingValue = getCaloriesPerServingValue(product);
  const usedServingSize = shouldUseServingSize(product, roleConfig);

  return {
    caloriesPerServing: scoreLowerIsBetter(
      usedServingSize ? caloriesPerServingValue : product.nutrition.caloriesPer100g,
      0,
      400,
    ),
    calorieDensity: scoreLowerIsBetter(product.nutrition.caloriesPer100g, 0, 500),
    protein: scoreHigherIsBetter(product.nutrition.proteinPer100g, 0, 20),
    sugar: scoreLowerIsBetter(product.nutrition.sugarPer100g, 0, 20),
    fiber: scoreHigherIsBetter(product.nutrition.fiberPer100g, 0, 6),
    fat: scoreLowerIsBetter(product.nutrition.fatPer100g, 0, 40),
    saturatedFat: scoreLowerIsBetter(product.nutrition.saturatedFatPer100g, 0, 20),
