...
const decodeHtmlEntities = (value: string): string => {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const nextValue = decoded.replace(HTML_ENTITY_PATTERN, (match: string, entity: string) => {
...
export const sanitizeProductText = (value?: string | null): string | null => {
  ...
  const sanitized = decodeHtmlEntities(value).normalize('NFKC').replace(/\s+/g, ' ').trim();
