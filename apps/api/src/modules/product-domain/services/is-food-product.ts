...
export const isFoodProduct = (product: NormalizedProduct): boolean => {
  ...
  const categoryText = [product.categories ?? '', ...product.category_tags].join(' ').toLowerCase();

  if (categoryText.length > 0) {
    const hasNonFoodCategory = NON_FOOD_KEYWORDS.some((keyword) => categoryText.includes(keyword));
    const hasFoodSignal =
