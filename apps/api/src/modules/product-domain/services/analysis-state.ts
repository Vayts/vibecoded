...
export interface BuildAnalysisSocketPayloadInput extends BuildAnalysisStateInput {
  scanId?: string;
  productId?: string;
  barcode?: string;
}
...
export const buildAnalysisResponse = (input: BuildAnalysisStateInput): AnalysisJobResponse => ({
...
export const parseStoredAnalysisResult = (value: unknown): ProductAnalysisResult | undefined => {
...
export const buildAnalysisResponseFromStoredState = (
  ...
  const productStatus = result ? 'completed' : storedStatus === 'failed' ? 'failed' : 'pending';
