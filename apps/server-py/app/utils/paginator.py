...
class Paginator:
    ...
    async def _get_items(self) -> list:
        ...
        data = await self.repository.execute(statement=statement, action=lambda result: result.scalars())
        return list(data)
    ...
    async def _get_total_count(self) -> int:
        count = await self.repository.session.scalar(select(func.count()).select_from(self.query.subquery()))
