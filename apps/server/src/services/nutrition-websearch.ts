...
const nutritionSearchResultSchema = z.object({
  ...
  source: z.string().nullable().describe('Source of the data (e.g. "OpenFoodFacts", "manufacturer website")'),
  nutrition: z
    .object({
...
function sanitizeNutritionValues(
  ...
  if (overCount >= 3) {
    console.log(`[NutritionSearch] ⚠️ Detected ${overCount} over-max values — scaling down by 10x (likely per-kg)`);
  ...
  for (const k of keys) {
    const v = result[k];
    if (v != null && v > (PER_100G_MAX[k] ?? Infinity)) {
      console.log(`[NutritionSearch] ⚠️ ${k}=${v} exceeds max ${PER_100G_MAX[k]} — setting to null`);
      result[k] = null;
    }
  }
  return result;
...
export const searchNutritionData = async (
  ...
  console.log(`[NutritionSearch] Starting web search for: "${searchQuery}" barcode=${barcode ?? 'none'}`);
  ...
  try {
    ...
    const facts: NutritionFacts = {
      calories: sane.energy_kcal_100g != null ? Math.round(sane.energy_kcal_100g) : null,
    ...
    const hasData = facts.calories != null && (facts.protein != null || facts.fat != null || facts.carbs != null);
    if (!hasData) {
      console.log(`[NutritionSearch] Incomplete data, discarding  ${elapsed}ms`);
      return null;
    }
