...
export const generateEmbeddingForText = async (text: string): Promise<number[] | null> => {
  const startedAt = Date.now();
  const embeddings = getEmbeddingsClient();

  if (!embeddings) {
    console.warn('[product-embedding] skip generate embedding because OPENAI_API_KEY is missing');
    return null;
  }
