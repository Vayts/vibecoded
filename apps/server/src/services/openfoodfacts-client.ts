...
const withTimeout = <T>(promise: Promise<T>, ms: number, label: string): Promise<T> =>
  new Promise<T>((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new OpenFoodFactsLookupError('TIMEOUT', `${label} timed out after ${ms}ms`)),
      ms,
    );
    promise.then(
...
export const lookupBarcode = async (barcode: string): Promise<NormalizedProduct | null> => {
  if (isCachedMiss(barcode)) {
    console.log(`[OFF] cache-miss hit for barcode=${barcode} — skipping API call`);
    return null;
  }
  ...
  try {
    response = await withTimeout(client.getProductV2(barcode), OFF_TIMEOUT_MS, 'OFF lookup');
  } catch (err) {
