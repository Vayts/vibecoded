...
export const findBestVectorMatchedProduct = async (
...
): Promise<{ product: NormalizedProduct; similarity: number; queryText: string } | null> => {
  ...
  if (!queryEmbedding) {
    console.log(`[product-vector-search] skip search because embedding generation returned null query="${queryText}"`);
    return null;
  }
  ...
  const candidateSummary = candidates
    .map((candidate, index) => `${index + 1}:${candidate.code}:${Number(candidate.similarity).toFixed(3)}:${candidate.product_name ?? 'unknown'}:${candidate.brands ?? 'unknown'}`)
    .join(' | ');
  console.log(
  ...
  if (!bestCandidate) {
    console.log(`[product-vector-search] no candidates query="${queryText}" elapsed=${Date.now() - startedAt}ms`);
    return null;
  }
