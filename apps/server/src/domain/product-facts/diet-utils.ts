...
const VEGAN_TOKENS = [
  'meat', 'beef', 'chicken', 'pork', 'fish', 'tuna', 'salmon', 'shellfish', 'shrimp',
  'dairy', 'milk', 'whey', 'butter', 'cheese', 'cream', 'yogurt', 'egg', 'honey',
  'gelatin', 'lard', 'tallow', 'bacon', 'ham', 'sausage', 'turkey', 'lamb', 'duck',
...
const HALAL_TOKENS = [
  'pork', 'bacon', 'ham', 'lard', 'wine', 'beer', 'rum', 'alcohol', 'sausage',
  'salami', 'prosciutto',
];

const KOSHER_TOKENS = [
  'pork', 'bacon', 'ham', 'shellfish', 'shrimp', 'crab', 'lobster', 'lard',
  'sausage', 'salami', 'prosciutto',
];

const GLUTEN_FREE_TOKENS = [
  'wheat', 'barley', 'rye', 'spelt', 'semolina', 'bulgur', 'malt', 'seitan',
  'farro', 'durum', 'gluten',
];
...
const NUT_FREE_TOKENS = [
  'peanut', 'almond', 'walnut', 'cashew', 'hazelnut', 'pistachio', 'macadamia',
  'pecan', 'tree nut', 'nut',
];
...
const DIET_TOKEN_MAP: Record<DietKey, { tokens: string[]; exclude?: RegExp[] }> = {
...
export const detectDietCompatibilityWithReasons = (
  ...
  const hasIngredients = product.ingredients.length > 0 || Boolean(product.ingredients_text);

  if (!hasIngredients) {
    const compatibility: DietCompatibility = {
      vegan: 'unclear', vegetarian: 'unclear', halal: 'unclear', kosher: 'unclear',
      glutenFree: 'unclear', dairyFree: 'unclear', nutFree: 'unclear',
    };
    const reasons: DietCompatibilityReasons = {
      vegan: 'Ingredients not available', vegetarian: 'Ingredients not available',
      halal: 'Ingredients not available', kosher: 'Ingredients not available',
      glutenFree: 'Ingredients not available', dairyFree: 'Ingredients not available',
  ...
  for (const [key, { tokens, exclude }] of Object.entries(DIET_TOKEN_MAP) as [DietKey, { tokens: string[]; exclude?: RegExp[] }][]) {
 ...
 */
export const detectDietCompatibility = (product: NormalizedProduct): DietCompatibility => {
