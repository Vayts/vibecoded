...
const TAB_BAR_GRADIENT_HEIGHT = 100;
...
export function CustomTabBar({ state, descriptors, navigation }: TabBarProps) {
  ...
  const renderTab = (route: TabBarProps['state']['routes'][number]) => {
    ...
    return (
      ...
      >
        <View
          className={`${isFocused ? 'bg-neutrals-200' : 'bg-transparent'} rounded-xl px-5 py-1.5`}
        >
  ...
  const renderScanAction = () => (
    ...
    >
      <View className="w-[52px] h-[52px] rounded-[26px] -top-6 absolute items-center justify-center bg-accent-500">
        <ScanBarcode color={COLORS.white} size={26} />
      </View>

      <Text
        className="text-sm relativeg font-semibold -mt-10"
        style={{ color: COLORS.neutrals900 }}
      >
        Scan
      </Text>
    </TouchableOpacity>
    ...
    >
      <LinearGradient
        ...
        style={{
          ...
          right: 0,
          bottom: 0,
          height: insets.bottom + TAB_BAR_GRADIENT_HEIGHT,
        }}
      />
