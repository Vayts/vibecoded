...
interface ComparisonResultState {
  liveResult: CompareProductsResponse | null;
...
interface ComparisonResultActions {
  ...
  resolveLiveComparison: (requestId: number, result: CompareProductsResponse) => void;
  setLiveResult: (result: CompareProductsResponse) => void;
}

export const useComparisonResultStore = create<ComparisonResultState & ComparisonResultActions>(
  (set, get) => ({
    liveResult: null,
